...
const fileSyncFrequency = 15 * time.Second
const cleanUpFrequency = 45 * time.Second
...
func (st *StenotypeThread) syncFilesWithDisk() {
	...
	for file := range st.listPacketFiles() {
		filename := file.Name()
...
func (st *StenotypeThread) listPacketFiles() <-chan os.FileInfo {
