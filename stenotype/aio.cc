...
class SingleFile {
 public:
  SingleFile(Output* file, const string& dirname, int64_t micros, int fd)
    : file_(file), fd_(fd), offset_(0), truncate_(-1),
  ...
  bool Closable() {
    return outstanding_.size() == 0 && truncate_ >= 0;
  }
  Error Close();
...
Output::Output(int aiops, int64_t initial_file_size)
    : ctx_(NULL), max_ops_(aiops),
      initial_file_size_(initial_file_size), current_(NULL) {
  CHECK_SUCCESS(SetUp());
}
...
Error Output::Rotate(const string& dirname, int64_t micros) {
  ...
  LOG_IF_ERROR(Errno(0 <= fallocate(fd, 0, 0, initial_file_size_)), "fallocate");
...
Error Output::Flush() {
  if (current_ == NULL) { return SUCCESS; }
  current_->RequestClose();
  current_ = NULL;
  while (files_.size()) {
